...
def vocab_tree_matcher(database_path, vocab_bin_path, match_list_path=None, ini_save_path=None, params=None):
    # Find and read template INI.
    ...
    # load vocabulary tree bin
    colmap_ini = override_ini_parameters(colmap_ini, {
        'VocabTreeMatching.vocab_tree_path': vocab_bin_path.replace('\\', '/')
    })
...
def model_aligner(path_to_model, path_to_geo_registered_model, path_to_text_file, ini_save_path=None, params=None):
    # Find and read template INI.
    # input_ini_file = os.path.join('template_inis', 'colmap_model_aligner.ini')
    # with open(input_ini_file, 'r') as f:
    #     colmap_ini = f.read()
    #
    # # Update some parameters, if requested.
    ...
    # # NB. Forward slashes in paths are expected by the GUI.
    # colmap_ini = override_ini_parameters(colmap_ini, {
    #     'input_path': path_to_model.replace('\\', '/'),
    #     'output_path': path_to_geo_registered_model.replace('\\', '/'),
    #     'ref_images_path': path_to_text_file.replace('\\', '/'),
    #     'robust_alignment': "1",
    ...
    # # Save INI file.
    # ini_save_path = save_ini(colmap_ini, ini_save_path)
    # colmap_command = [colmap_bin, "model_aligner", "--project_path", ini_save_path]
